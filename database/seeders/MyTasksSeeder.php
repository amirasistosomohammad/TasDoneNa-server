...
/**
 * Seeds tasks for the My Tasks component (personnel/officer view).
 * Creates varied tasks for approved officers to test listing, filtering,
 * status changes, pagination, search, and responsiveness.
 *
 * Test users (password: password):
 * - personnel.active.1@tasdonena.test — 15 tasks (mix of statuses)
 * - personnel.active.2@tasdonena.test — 5 tasks
 */
class MyTasksSeeder extends Seeder
{
    public function run(): void
    {
        $officer1 = User::where('email', 'personnel.active.1@tasdonena.test')->first();
        $officer2 = User::where('email', 'personnel.active.2@tasdonena.test')->first();

        if (!$officer1) {
            $this->command->warn('MyTasksSeeder: personnel.active.1@tasdonena.test not found. Run PersonnelDirectorySeeder first.');
            return;
        }

        $existingCount = Task::where('created_by', $officer1->id)->count();
        if ($existingCount >= 10) {
            $this->command->info('MyTasksSeeder: Officer 1 already has tasks. Skipping to avoid duplicates. Run migrate:fresh --seed for a clean slate.');
            return;
        }

        foreach ($this->myTasksData($officer1->id) as $data) {
            Task::create($data);
        }

        if ($officer2) {
            $existingCount2 = Task::where('created_by', $officer2->id)->count();
            if ($existingCount2 < 5) {
                foreach ($this->officer2TasksData($officer2->id) as $data) {
                    Task::create($data);
                }
            }
        }
     ...
     */
    private function myTasksData(int $userId): array
    {
        $now = now();
        $base = [
            'created_by' => $userId,
     ...
     * @return array<int, array<string, mixed>>
     */
    private function officer2TasksData(int $userId): array
    {
        $now = now();
        $base = [
            'created_by' => $userId,
        ...
        return [
            array_merge($base, [
                'title' => 'Review student portfolios for Q1',
                'description' => 'Check and provide feedback on learner portfolios.',
                'mfo' => 'MFO 2: Learner Development',
                'kra' => 'KRA 2: Learning Environment',
                'kra_weight' => 20.00,
                'objective' => 'Assess portfolio completeness and quality.',
                'movs' => ['Feedback form', 'Portfolio checklist'],
                'due_date' => $now->copy()->addDays(5)->format('Y-m-d'),
                'cutoff_date' => $now->copy()->addDays(3)->format('Y-m-d'),
                'status' => 'pending',
                'priority' => 'medium',
                'timeline_start' => $now->copy()->format('Y-m-d'),
                'timeline_end' => $now->copy()->addDays(5)->format('Y-m-d'),
                'performance_criteria' => ['All portfolios reviewed'],
            ]),
            array_merge($base, [
                'title' => 'Prepare COT observation materials',
                'description' => 'Get ready for classroom observation. Align lesson with RPMS indicators.',
                'mfo' => 'MFO 1: Teaching-Learning Process',
                'kra' => 'KRA 1: Content Knowledge and Pedagogy',
                'kra_weight' => 25.00,
                'objective' => 'Demonstrate teaching competence.',
                'movs' => ['Lesson plan', 'PowerPoint', 'Activity sheets'],
                'due_date' => $now->copy()->addDays(10)->format('Y-m-d'),
                'cutoff_date' => $now->copy()->addDays(8)->format('Y-m-d'),
                'status' => 'pending',
                'priority' => 'high',
                'timeline_start' => $now->copy()->subDays(3)->format('Y-m-d'),
                'timeline_end' => $now->copy()->addDays(10)->format('Y-m-d'),
                'performance_criteria' => ['Meets RPMS standards'],
            ]),
            array_merge($base, [
                'title' => 'Submit school form 2 for enrollment',
                'description' => 'Encode and submit SF2 to division office.',
                'mfo' => 'MFO 5: Plus Factor',
                'kra' => 'KRA 5: Professional Growth',
                'kra_weight' => 15.00,
                'objective' => 'Accurate enrollment reporting.',
                'movs' => ['SF2 printout', 'Submission receipt'],
                'due_date' => $now->copy()->subDays(5)->format('Y-m-d'),
                'cutoff_date' => $now->copy()->subDays(7)->format('Y-m-d'),
                'status' => 'completed',
                'priority' => 'high',
                'timeline_start' => $now->copy()->subDays(14)->format('Y-m-d'),
                'timeline_end' => $now->copy()->subDays(5)->format('Y-m-d'),
                'performance_criteria' => ['Submitted on time'],
            ]),
            array_merge($base, [
                'title' => 'Organize classroom bulletin board',
                'description' => 'Update bulletin board with current themes and student work.',
                'mfo' => 'MFO 2: Learner Development',
                'kra' => 'KRA 2: Learning Environment',
                'kra_weight' => 20.00,
                'objective' => 'Create engaging learning environment.',
                'movs' => ['Photos of bulletin board'],
                'due_date' => $now->copy()->addDays(2)->format('Y-m-d'),
                'cutoff_date' => $now->copy()->addDays(1)->format('Y-m-d'),
                'status' => 'pending',
                'priority' => 'low',
                'timeline_start' => $now->copy()->format('Y-m-d'),
                'timeline_end' => $now->copy()->addDays(2)->format('Y-m-d'),
                'performance_criteria' => ['Theme-appropriate', 'Student work displayed'],
            ]),
            array_merge($base, [
                'title' => 'Attend PTA general assembly',
                'description' => 'Participate and document PTA assembly. Present class concerns.',
                'mfo' => 'MFO 3: School and Community',
                'kra' => 'KRA 3: Diversity of Learners',
                'kra_weight' => 15.00,
                'objective' => 'Foster parent engagement.',
                'movs' => ['Attendance', 'Minutes'],
                'due_date' => $now->copy()->addDays(7)->format('Y-m-d'),
                'cutoff_date' => $now->copy()->addDays(5)->format('Y-m-d'),
                'status' => 'pending',
                'priority' => 'medium',
                'timeline_start' => $now->copy()->format('Y-m-d'),
                'timeline_end' => $now->copy()->addDays(7)->format('Y-m-d'),
                'performance_criteria' => ['Attendance documented'],
            ]),
        ];
    }
}
